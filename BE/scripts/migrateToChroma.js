...
const DATA_FILE_PATH = path.join(process.cwd(), '../pharmaceutical_rag_data.json');
const COLLECTION_NAME = process.env.CHROMA_DATABASE || 'RAG-TemanPulih';
    ...
    for (const item of data) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        metas.push({
          source_id: item.id,
          chunk_index: i,
          disease_name: item.metadata?.disease_name || '',
          has_drugs: item.metadata?.has_drugs || false
        });
      }
